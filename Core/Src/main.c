...
static int counterPacket=1;
  ...
  const uint32_t transmission_delay = 1500; // 500 ms delay before transmission
...
volatile uint8_t result_dec = 255;
    		...
    		if (byte == 0x3C) {
    			...
    			if (packetIndex > 4 && packetBuffer[1] == 0x98) {
    				...
    				if (command1 == 0x01 && command2 == 0x01) {
    					...
    					memcpy(FinalData, packetBuffer, sizeof(packetBuffer));
    					...
    					/*
    					    					...
    					    					case 1: //Module one
    					    						 ...
    					    						 HAL_UART_Transmit(&huart2, (uint8_t *)msgStatus, strlen(msgStatus), HAL_MAX_DELAY);
    					    						 if (!operationStarted) {
    					    							 HAL_TIM_Base_Start_IT(&htim3);  // Start packet transmission
    					    							 operationStarted = 1;
    					    							 startpacket_transission();
    					    						 }
    					    						 break;
    					    					case 2:  // Module 2
...
void AppMasterBoard1(){
	sprintf(msgStatus, "\r\n Master mode on \r\n");
	HAL_UART_Transmit(&huart2, (uint8_t *)msgStatus, strlen(msgStatus), HAL_MAX_DELAY);
	//HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_RESET);

	sprintf(msgStatus, "\r\n Waiting for start_module flag \r\n");
	HAL_UART_Transmit(&huart2, (uint8_t *)msgStatus, strlen(msgStatus), HAL_MAX_DELAY);
	...
	while(1){
		...
		if (!operationStarted && currentPinState == GPIO_PIN_SET && lastPinState == GPIO_PIN_RESET){// && start_module {
			HAL_Delay(debounceDelay); // Small debounce delay to ensure stable signal
			/*HAL_TIM_Base_Start_IT(&htim3);
			operationStarted = 1;
			sprintf(msgStatus, "\r\n Timer started \r\n");
			HAL_UART_Transmit(&huart2, (uint8_t *)msgStatus, strlen(msgStatus), HAL_MAX_DELAY);
			...
			// Check if confirmation was received before transmitting
			//if (confirmationReceived == 1) {

			//Read of pin to transmit
			GPIO_PinState pinState = HAL_GPIO_ReadPin(GPIO_PORT_ENTRY, GPIO_PIN_ENTRY);
			// Mod to start with User button pressed rather than Gpio entry
			if (pinState == GPIO_PIN_SET){ //pinState == GPIO_PIN_SET push button HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET
...
/*
		    ...
		    // Update lastPinState for the next cycle
		    lastPinState = currentPinState;
		    //HAL_GPIO_TogglePin(Timer_GPIO_Port, Timer_Pin);
		    //HAL_GPIO_WritePin(Timer_GPIO_Port, Timer_Pin, GPIO_PIN_SET);
		    timer_counter = __HAL_TIM_GET_COUNTER(&htim3);
		    //HAL_Delay(1);
		   // HAL_GPIO_WritePin(Timer_GPIO_Port, Timer_Pin, GPIO_PIN_RESET);
...
void AppMasterBoard(){
	...
	while(1){
		...
		if (transmission_state != last_transmission_state){
		    ...
		    sprintf(msgStatus, "\r\n Result decoded: %d, Current state: %d \r\n", result_dec, transmission_state);
		    //HAL_UART_Transmit(&huart2, (uint8_t *)msgStatus, strlen(msgStatus), HAL_MAX_DELAY);
		}
		last_transmission_state = transmission_state;
		    ...
		    switch (transmission_state) {
		        ...
		        case MONITOR_ORDER_MODULES:
		        	 		//
		        	uint8_t next = next_module(result_dec);
